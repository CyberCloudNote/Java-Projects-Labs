...
public class Controller {
	...
	public Controller() {
		...
		do {
			...
			if(controlNum == 1) {
				...
				if(controlNum == 1) {
					if(stockSelect == 1)
						Controller.sellLIFO(googList, quantity, currPrice);
					else
						Controller.sellLIFO(amazList, quantity, currPrice);
				}
				else {
					if(stockSelect == 1)
						Controller.sellFIFO(googList, quantity, currPrice);
					else
						Controller.sellFIFO(amazList, quantity, currPrice);
				}
			}
	...
	public static void sellLIFO(LinkedList<Stock> list, int numToSell, int priceNow) {
	    ...
	    if (list.size() < numToSell)
	    {
	    	System.out.print("You do not have that many stocks.");
	    }
	    else
	    {
	...
	public static void sellFIFO(LinkedList<Stock> list, int numToSell, int priceNow) {
